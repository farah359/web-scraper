...
def extract_product(p):
    ...
    return {
        "nom":            p.get("name", ""),
        "categorie":      current_category,   # ✅ depuis notre liste PAGES
        "sousCategorie":  current_sous_cat,   # ✅ depuis notre liste PAGES (fiable)
        "image":          image,
...
def scrape():
    global current_category, current_sous_cat   # ✅ les deux globals
    ...
    with sync_playwright() as p:
        ...
        # ✅ CORRIGÉ : décompactage 3 valeurs
        for cat_name, sous_cat, url in pages_to_scrape:
            current_category = cat_name
            current_sous_cat = sous_cat

            print(f"\n{'='*70}")
            print(f"🔥  {cat_name}  →  {sous_cat}")
            print(f"{'='*70}")
            ...
            for i in range(20):
                ...
                time.sleep(2.5)
                if i % 5 == 0:
                    print(f"  ↕ scroll {i+1}/20 — {len(products)} produits au total")
    ...
    stats = Counter(f"{p['categorie']} / {p['sousCategorie']}" for p in products)
    print("\n📊 Répartition :")
